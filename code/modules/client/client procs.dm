...
/client/New(TopicData)
	...
	if(holder)
		add_admin_verbs()
		admin_memo_show()
		if((global.comms_password == "default_pwd" || length(global.comms_password) <= 6) && global.comms_allowed) //It's the default value or less than 6 characters long, but it somehow didn't disable comms.
...
/client/proc/sync_client_with_db()
	...
	if(query_watch.NextRow())
		message_admins("<font color='red'><B>Notice: </B></font><font color='blue'>User [src.key] is currently being watched, please keep an eye on them. Watch reason: [query_watch.item[2]]</font>")
		send2irc("Watch Alert", "[src.ckey] is currently being watched and has joined the server! - Watch reason: [query_watch.item[2]]")
